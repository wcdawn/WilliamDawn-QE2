...
\label{sec:summary}
  ...
  \subsection{Multigroup Neutron Diffusion and \glsentrylong{nem}}
    ...
    \eref{eq:transverse_multigroup_diffusion} that the node indices (i.e.
    ...
    \subsubsection{Choice of Physics-Based Preconditioner}
      ...
      the number of Krylov iterations required to solve a linear system and it
      can be shown that if one preconditions a matrix with its inverse, the
      Krylov method will converge in one iteration \cite{textbookkelley}. While
      ...
      \citeauthor{qe2paper}. Instead, the authors design a new preconditioner
